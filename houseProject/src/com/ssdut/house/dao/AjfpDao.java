...
public class AjfpDao extends BaseDao{
	private PageBeanDao<Case> pageBeanDao=new PageBeanDao<Case>();
	public PageBean<Case> getPageBean(int page,int pageSize){
		String sql="select * from allcase where caseState='审核通过'";
		return pageBeanDao.getPageBean(sql, page, pageSize,Case.class);
	}
	public void dispatch(Case c) {
		// TODO Auto-generated method stub
		utils.getConnection();
		String sql="update allcase set undertakeUserId=?,caseState=? where id=?";
		List<Object> params=new ArrayList<Object>();
		params.add(c.getUndertakeUserId());
